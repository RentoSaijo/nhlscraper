...
`team_game_report()` returns the game-level version of a selected team
stats report, with one row per team per game and the category-specific
metric columns exposed by the stats API.
