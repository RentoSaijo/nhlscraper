# Access various reports for a season, game type, and category for all the skaters by game

`skater_game_report()` returns the game-level version of a selected
skater stats report, with one row per player per game and
category-specific metrics. Regular-season calls are paged by month to
avoid oversized API requests.

## Usage

``` r
skater_game_report(
  season = season_now(),
  game_type = game_type_now(),
  category = "summary"
)
```

## Arguments

- season:

  integer in YYYYYYYY (e.g., 20242025); see
  [`seasons()`](https://rentosaijo.github.io/nhlscraper/reference/seasons.md)
  for reference

- game_type:

  integer in 1:3 (where 1 = pre-season, 2 = regular season, 3 =
  playoff/post-season) OR character of 'pre', 'regular', or
  playoff'/'post'; see
  [`seasons()`](https://rentosaijo.github.io/nhlscraper/reference/seasons.md)
  for reference; most functions will NOT support pre-season

- category:

  character (e.g., 'puckPossessions'); see
  [`skater_report_configurations()`](https://rentosaijo.github.io/nhlscraper/reference/skater_report_configurations.md)
  for reference

## Value

data.frame with one row per game per player

## Examples

``` r
# May take >5s, so skip.
possession_skater_game_report_playoff_20212022 <- 
  skater_game_report(
    season    = 20212022, 
    game_type = 3, 
    category  = 'puckPossessions'
  )
```
