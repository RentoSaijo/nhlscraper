...
def fetch_game(game_id: int) -> list[dict[str, object]]:
    req = Request(report_url(game_id), headers={"User-Agent": "nhlscraper-shot-audit/1.0"})
    with urlopen(req, timeout=20) as resp:
        doc = html.fromstring(resp.read())

    events: list[dict[str, object]] = []
    for row in doc.xpath(".//tr"):
        cells = [cell.text_content().replace("\xa0", " ").strip() for cell in row.xpath("./th|./td")]
...
def main() -> int:
    ...
    all_events.sort(key=lambda x: (int(x["gameId"]), str(x["period"]), x["secondsElapsedInPeriod"] or -1, int(x["htmlEventNumber"])))
    fieldnames = ["gameId", "period", "secondsElapsedInPeriod", "htmlEventNumber", "htmlEventCode", "eventTypeDescKey", "description"]
    ...
    with TOTALS_OUT.open("w", newline="") as f:
        writer = csv.DictWriter(f, fieldnames=["source", "rows", "G", "saved", "iSF", "missed", "iFF", "blocked", "iCF", "failures"])
        writer.writeheader()
        writer.writerow(
            {
    ...
    if failures:
        ...
        print(f"Completed with {len(failures)} failures; see {failures_out}", file=sys.stderr)
    else:
