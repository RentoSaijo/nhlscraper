...
cmd_file <- sub("^--file=", "", commandArgs(FALSE)[grepl("^--file=", commandArgs(FALSE))][1] %||% "")
...
regular_attempts <- pbp |>
  filter(gameTypeId == 2, eventTypeDescKey %in% shot_types)
write.csv(
  pbp |>
    filter(gameTypeId == 2) |>
    distinct(gameId) |>
...
pbp_filters <- bind_rows(
  strength_even = regular_attempts |> filter(strengthState == "even-strength") |> count_attempts(),
  skater_5v5 = regular_attempts |> filter(skaterCountFor == 5, skaterCountAgainst == 5) |> count_attempts(),
  code_1551 = regular_attempts |> filter(situationCode == "1551") |> count_attempts(),
  code_1551_no_empty = regular_attempts |>
    filter(situationCode == "1551", !isEmptyNetFor, !isEmptyNetAgainst) |>
    count_attempts(),
  .id = "source"
)
...
reason_totals <- pbp_1551 |>
  mutate(reason = ifelse(is.na(reason), "<NA>", reason)) |>
  count(eventTypeDescKey, reason, name = "n") |>
  arrange(eventTypeDescKey, desc(n), reason)
...
write.csv(official_team_summary, file.path(out_dir, "official_team_summaryshooting_totals.csv"), row.names = FALSE)
...
parse_html_attempt_counts <- function(game_id, home_id, away_id, home_abbrev, away_abbrev) {
  ...
  if (is.null(doc)) {
    return(tibble(gameId = game_id, eventTypeDescKey = NA_character_, n = NA_integer_, fetched = FALSE))
  }
  ...
  if (!n_out) {
    return(tibble(gameId = game_id, eventTypeDescKey = character(), n = integer(), fetched = TRUE))
  }
...
if (run_r_html && !file.exists(html_events_file)) {
  team_lookup <- teams() |> select(teamId, teamTriCode)
  game_sides <- pbp |>
    filter(gameTypeId == 2, !is.na(eventOwnerTeamId), !is.na(isHome)) |>
    group_by(gameId) |>
    summarise(
      homeId = first(eventOwnerTeamId[isHome]),
      awayId = first(eventOwnerTeamId[!isHome]),
      .groups = "drop"
    ) |>
    left_join(team_lookup, by = c("homeId" = "teamId")) |>
    rename(homeAbbrev = teamTriCode) |>
    left_join(team_lookup, by = c("awayId" = "teamId")) |>
    rename(awayAbbrev = teamTriCode) |>
    arrange(gameId)
