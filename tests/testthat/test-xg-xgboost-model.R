test_that("xG partitioning covers the six shot situations", {
  shots <- data.frame(
...
test_that("xG partitioning routes uncategorizable rows to sd", {
  shots <- data.frame(
...
test_that("xG partitioning does not force sd on missing situationCode alone", {
  shots <- data.frame(
...
test_that("xG target-season selection uses bundled vintages", {
...
test_that("xG categorical encoding keeps known levels out of new bucket", {
...
test_that("xG matrix baking follows frozen preprocessing spec", {
  spec <- nhlscraper:::.xg_load_bundle()$preprocess_specs$v20262027_sd
  ...
  expect_equal(dim(mat), c(2L, length(spec$feature_names)))
  expect_equal(colnames(mat), spec$feature_names)
  expect_equal(unname(mat[, "shotType_wrist"]), c(1, 0))
  expect_equal(unname(mat[, "shotType_new"]), c(0, 1))
  expect_false(anyNA(mat))
})

test_that("xG XGBoost scoring returns finite probabilities with sparse inputs", {
  df <- data.frame(
